...
function enviarABasura(cartaElemento, canecaElemento) {
  const rectCarta = cartaElemento.getBoundingClientRect();
  const rectCaneca = canecaElemento.getBoundingClientRect();

  const distanciaX = rectCaneca.left + rectCaneca.width / 2 - (rectCarta.left + rectCarta.width / 2);
  const distanciaY = rectCaneca.top + rectCaneca.height / 2 - (rectCarta.top + rectCarta.height / 2);

  cartaElemento.style.transform = `translate(${distanciaX}px, ${distanciaY}px) scale(0.05) rotate(20deg)`;
  cartaElemento.classList.add("desechada");
}

function compararCartas() {
  const primera = cartasVolteadas[0];
  const segunda = cartasVolteadas[1];
  const caneca = document.getElementById("canecaBasura");
  ...
  if (primera.dataset.simbolo === segunda.dataset.simbolo) {
    ...
    setTimeout(function () {
      if (caneca) caneca.classList.add("abierta");
      enviarABasura(primera, caneca);
      enviarABasura(segunda, caneca);
    }, 300);

    setTimeout(function () {
      if (caneca) caneca.classList.remove("abierta");
    }, 1100);
